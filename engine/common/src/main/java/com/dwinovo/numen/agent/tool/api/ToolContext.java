 ...
 * game time (the basis for a deadline), and the model-requested {@code timeout_seconds}
 ...
 *
 * <h2>The {@code timeout_seconds} cap</h2>
 * Every tool schema carries an optional {@code timeout_seconds} the model may fill to
 ...
 * own self-estimated budget to {@code min(estimate, now + timeout_seconds*20)}. Because
...
public record ToolContext(String toolCallId, long gameTime, int maxSeconds) {
    ...
    /** Back-compat convenience: a context with no {@code timeout_seconds} cap. */
     ...
     * When the model supplied a {@code timeout_seconds} cap for this call, the returned
