...
public final class PathVizRenderer {
    ...
    private static void line(VertexConsumer vc, PoseStack.Pose pose,
        ...
        vc.addVertex(pose, (float) x1, (float) y1, (float) z1).setColor(color).setNormal(pose, n.x(), n.y(), n.z()).setLineWidth(LINE_WIDTH);
        vc.addVertex(pose, (float) x2, (float) y2, (float) z2).setColor(color).setNormal(pose, n.x(), n.y(), n.z()).setLineWidth(LINE_WIDTH);
    }
}
