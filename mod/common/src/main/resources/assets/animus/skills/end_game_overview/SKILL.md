...
3. **Verify each phase's "done when" with `get_storage` / `get_self_status`** before marking it `completed` — never assume an item is in your inventory.
...
|---|---|---|---|---|
| 1 | Tier up to diamond | `tier_progression` | Diamond pickaxe + diamond sword, iron-or-better armor, bow + 64 arrows, 32+ cooked food, 64+ cobblestone | "Diamonds!" |
...
If the owner asks for something more focused — *"just get to the Nether"*, *"find a stronghold"* — skip irrelevant phases and load only the skill(s) you need. Phases assume the previous phase's inventory; check `get_storage` and backfill gaps instead of blindly starting from phase 1.
